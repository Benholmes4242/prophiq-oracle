...
interface Body { event_id?: string; force?: boolean; }
...
Deno.serve(async (req) => {
  ...
  // Reuse fresh prediction unless forced
  if (!body.force) {
  ...
  const prompt = adapter.buildPrompt(event as DomainEvent, outcomes as EventOutcome[]);
  ...
  // Mark prior predictions stale
  await supabase.from("predictions").update({ is_current: false })
    .eq("event_id", body.event_id).eq("mode", event.mode === "both" ? "prediction" : event.mode);

  const persistedMode = event.mode === "both" ? "prediction" : event.mode;
